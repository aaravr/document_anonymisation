"""Restore canonical_registry.py with the improved sweep."""
content = '''"""Document-level canonical entity registry."""
        ...
        for cluster in self._clusters:
            ...
            for surface, allow_suffix in candidates:
                if not surface or len(surface.strip()) < 4:
                    continue
                if allow_suffix:
                    pat = r"\\b" + re.escape(surface) + _LEGAL_SUFFIX_PATTERN + r"\\b"
                else:
...
'''
import os, ast
target = os.path.join(os.path.dirname(__file__), 'idp_anonymiser', 'detection', 'canonical_registry.py')
